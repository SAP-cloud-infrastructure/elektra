  ...
  module Authentication
    class AuthSession
        ...
        def check_authentication(controller, scope_and_options = {})
          ...
          if session.authenticated?
            if !two_factor or two_factor_cookie_valid?(controller)
              ...
              controller.redirect_to controller.two_factor_path(url_options)
              nil
            end
          else
            # not authenticated!
      ...
      def redirect_to_login_form_url
        ...
        if @scope[:domain_name]
          url_helpers.login_path(url_params.merge(domain_name: @scope[:domain_name]))
        else
          @url_helpers.new_session_path(url_params.merge(domain_id: @scope[:domain]))
        end
      end
