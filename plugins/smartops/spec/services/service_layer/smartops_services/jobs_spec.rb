...
RSpec.describe ServiceLayer::SmartopsServices::Jobs do
  ...
  before do
    # Stub the MOCK_DATA constant to use our test data
    stub_const("ServiceLayer::SmartopsServices::Jobs::MOCK_DATA", mock_jobs_data)
    
    # Mock the sleep method to avoid delays during testing
    allow(test_class).to receive(:sleep)
  end
  ...
  describe '#list_jobs' do
    ...
    context 'performance and behavior' do
      it 'does not call sleep during tests' do
        test_class.list_jobs
        
        expect(test_class).to have_received(:sleep)
      end
