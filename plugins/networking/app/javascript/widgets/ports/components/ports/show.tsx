...
const ShowPortModal: React.FC<ShowPortModalProps> = ({
  ...
  useEffect(() => {
    ...
    loadSubnetsOnce()
    loadSecurityGroupsOnce()
  }, [loadPort, loadNetworksOnce, loadSubnetsOnce, loadSecurityGroupsOnce, port, networks, subnets, securityGroups])
  ...
  const renderSecurityGroups = (portData: Port) => {
    const items = portData.security_groups.map((groupId) => {
      ...
      const securityGroup = securityGroups.items.find((i) => i.id === groupId)
