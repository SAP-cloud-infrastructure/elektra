...
describe MonsoonOpenstackAuth::SessionsController, type: :controller do
  ...
  describe 'POST #consume_auth_token with raw SSO token' do
    context 'with valid raw token from browser SSO fetch' do
      before do
        allow(MonsoonOpenstackAuth::Authentication::AuthSession)
          .to receive(:create_from_auth_token)
          .with(controller, valid_token)
      ...
      it 'creates auth session and returns JSON with redirect URL' do
        post :consume_auth_token, params: {
          auth_token: valid_token,
          raw: '1',
          domain_fid: domain_id,
        ...
        expect(body['redirect_to']).to be_present
      end

      it 'uses after_login param when provided' do
        post :consume_auth_token, params: {
          auth_token: valid_token,
          raw: '1',
          domain_fid: domain_id,
        ...
        body = JSON.parse(response.body)
        expect(body['ok']).to eq(true)
        expect(body['redirect_to']).to eq(after_login_url)
      end

      it 'calls create_from_auth_token with the raw token' do
        expect(MonsoonOpenstackAuth::Authentication::AuthSession)
          .to receive(:create_from_auth_token)
          .with(controller, valid_token)
          .and_return(mock_auth_session)
      ...
      it 'does not attempt to decode the token via MessageVerifier' do
        expect(ActiveSupport::MessageVerifier).not_to receive(:new)
    ...
    context 'with invalid raw token' do
      before do
        allow(MonsoonOpenstackAuth::Authentication::AuthSession)
          .to receive(:create_from_auth_token)
          .with(controller, invalid_token)
    ...
    context 'with failed login session for raw token' do
      before do
        allow(MonsoonOpenstackAuth::Authentication::AuthSession)
          .to receive(:create_from_auth_token)
          .with(controller, valid_token)
    ...
    context 'backward compatibility' do
      it 'still redirects for non-raw POST requests' do
        allow(MonsoonOpenstackAuth::Authentication::AuthSession)
          .to receive(:create_from_auth_token)
          .with(controller, valid_token)
          .and_return(mock_auth_session)
      ...
      it 'still decodes token for GET requests' do
  ...
  describe 'safe_redirect_url?' do
    let(:username) { 'D123456' } # natural user pattern — works regardless of ENFORCE_NATURAL_USER_LOGIN
    let(:password) { 'password123' }
    ...
    context 'with password_session_auth_allowed disabled' do
      ...
      context 'with valid credentials (normal login form)' do
        before do
          allow(MonsoonOpenstackAuth::Authentication::AuthSession)
            .to receive(:create_from_login_form)
            .and_return(mock_auth_session)
      ...
      context 'with valid credentials (password sync form)' do
        before do
          allow(MonsoonOpenstackAuth::Authentication::AuthSession)
            .to receive(:create_from_login_form)
            .and_return(mock_auth_session)
        ...
        it 'shows sync-specific notice when password_sync param is present' do
          post :create, params: {
            domain_fid: domain_id,
            username: username,
            password: password,
